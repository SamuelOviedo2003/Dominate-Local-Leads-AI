 ...
 * Reset Password Page
 * 
 * This page handles the password reset flow after users click the email link.
 * It expects a 'code' parameter in the URL which is exchanged for a session.
 * 
 * Flow:
 * 1. User clicks email link with code parameter
 * 2. This page exchanges code for session
 * 3. User can then update their password
...
export default async function ResetPasswordPage() {
  ...
  // If authenticated user but not in recovery mode, redirect to dashboard
  if (user) {
    // In a recovery flow, we want to allow password updates even if logged in
    // The ResetPasswordForm component will handle the code exchange
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-blue-800 via-brand-slate-700 to-brand-orange-900 animate-gradient flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-brand-blue-300/5 rounded-full blur-3xl animate-float" style={{ animationDelay: '2s' }}></div>
      </div>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        {/* Glass Morphism Reset Password Form */}
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-8 animate-scale-in relative group hover:bg-white/15 transition-all duration-300">
          {/* Subtle glow effect */}
          <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-brand-orange-400/20 via-brand-orange-300/20 to-brand-blue-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl"></div>
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-32 left-20 w-1.5 h-1.5 bg-brand-blue-300/25 rounded-full animate-float" style={{ animationDelay: '0.5s' }}></div>
