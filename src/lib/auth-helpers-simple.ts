...
async function getUserAccessibleBusinesses(
  ...
  if (userRole === 0) {
    ...
    if (error) throw new Error('Failed to fetch businesses for super admin')
    ...
    if (error) throw new Error('Failed to fetch user businesses')
...
export async function getAuthenticatedUserFromRequest(): Promise<AuthUser | null> {
  try {
    // Check if cookie-based auth is enabled
    ...
    if (useCookieAuth) {
      ...
      const { data: { user }, error } = await supabase.auth.getUser()
      if (error || !user) {
        console.log('[AUTH] No authenticated user found in cookies:', error?.message)
        return null
      }
      ...
      if (profileError || !profile) {
        console.log('[AUTH] Profile not found for user:', user.id, profileError?.message)
        return null
      }
      ...
      if (!token) {
        ...
        console.log('[AUTH] No token found in headers for localStorage mode')
        return null
      }
      return getAuthenticatedUser(token)
    }
  } catch (error) {
    console.error('[AUTH] Error getting authenticated user from request:', error)
    return null
  }
}
