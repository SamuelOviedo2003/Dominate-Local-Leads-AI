  ...
  if (!hasPlatformBreakdown) {
    return (
      <div className="mb-8 bg-gray-50 border border-gray-200 rounded-lg p-6">
        <div className="text-gray-500 text-center">No platform data available for the selected period</div>
      </div>
    )
  }
      ...
      {platformSpendMetrics.platformSpends.map((platform, index) => (
        ...
        >
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">{platform.platform}</p>
              <p className="text-2xl font-bold text-gray-900">{formatCurrency(platform.spend)}</p>
            </div>
          </div>
        </div>
      ))}
